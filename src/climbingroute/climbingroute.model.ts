...
export const create = async (supabase: SupabaseClient<Database>, climbingroute: ClimbingRoute): Promise<ClimbingRoute> => {
...
export const getAll = async (supabase: SupabaseClient<Database>): Promise<ClimbingRoute[]> => {
    const { data, error } = await supabase
        .from('climbing_routes')
        .select();
    ...
    const parsedData = data.map(route => {
        return {
            ...route,
            pitches: Array.isArray(route.pitches) ? route.pitches.map((pitch: Json) => {
...
export const getById = async (supabase: SupabaseClient<Database>, id: string): Promise<ClimbingRoute | null> => {
    const { data, error } = await supabase
        .from('climbing_routes')
        .select()
        .eq('id', id)
    ...
    const parsedData = {
        ...data,
        pitches: Array.isArray(data.pitches) ? data.pitches.map((pitch: Json) => {
            const validationResult = ClimbingPitch.safeParse(pitch);
            if (!validationResult.success) {
                throw new Error('Invalid JSON structure');
            }
...
export const update = async (supabase: SupabaseClient<Database>, id: string, climbingroute: ClimbingRoute): Promise<ClimbingRoute> => {
    const { data, error } = await supabase
        .from('climbing_routes')
