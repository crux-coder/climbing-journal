...
export const getById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const template: Template | null = await TemplateModel.getById(supabase, templateId);

        if (!template) {
            next(new AppError('Template not found', 404, 'NOT_FOUND', 'Template not found with the provided ID.'));
        }
...
export const update = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // Middleware already validated the ID, if we reach this point, we can safely assume that the ID is a string
        const templateId: string = req.params.id as string;
        const template: Template = await TemplateModel.update(supabase, templateId, req.body);
