...
const createFeatureFiles = (featureDir, featureName) => {
    ...
    files.forEach(file => {
        ...
        const templateFileName = file === 'index.ts' ? 'index.ts' : `template.${file.split('.')[1]}.ts`;
        const templatePath = path.join(__dirname, '..', 'src', 'template', templateFileName);
        createFileFromTemplate(templatePath, filePath, featureName);
    });

    console.log(`Feature '${featureName}' created successfully with the following files:`);
